...
var xWords = {
 ...
 Create: function(height, width, arrayOfWords){
	 ...
	 do {
		 ...
		 if (!this.bestFit || (this.bestFit && this.bestFit.WordGroupsCount > this.GetNumberOfWordGroups())){
			this.bestFit = null;
			this.bestFit = new AlternativeGrid(
				this.Grid.slice(),
				JSON.parse(JSON.stringify(this.Words)), // Clone entire array
	 ...
	 return {
		 words: this.bestFit.Words.filter(word=>word.chosenPosition),
		 grid: this.bestFit.Grid
	 };
	},
